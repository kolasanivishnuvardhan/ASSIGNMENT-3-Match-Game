...
class App extends Component {
  ...
  decrementTime = () => {
    this.setState(prevState => {
      if (prevState.time !== 1) {
        ...
        return {time: 0, gameEnd: true}
        // game has end show the scorecard component
      }
    })
  }
  ...
  onClickThumbnail = imageUrl => {
    ...
    // check weather the imageUrl and imageToBeMatched are same to increment the score
    if (imageUrl === imageToBeMatched) {
      // increment the score and generate new image
      this.setState(prevState => {
        const newImage = this.getRandomImage()
        return {score: prevState.score + 1, imageToBeMatched: newImage}
      })
    } else {
      //end game because user clicks on wrong image
      clearInterval(this.timerId)
  ...
  getRandomImage = () => {
    ...
    do {
      ...
      newImage = imagesList[randomIndex].imageUrl
    } while (newImage ===imageToBeMatched)
  ...
  render() {
    const {tabId, imageToBeMatched, score, time, gameEnd} = this.state
    const hideOrDisplay = gameEnd ? 'hide-images' : ''
    const filteredList = imagesList.filter(eachImage =>
      eachImage.category.includes(tabId),
    )
    return (
      <div className="match-game-bg-container">
          ...
          <ul className="score-and-timer-container">
            ...
            <li className="timer-container">
              <img
                ...
                className="timer-img"
              />
              <p className="time-in-sec">{time} sec</p>
            </li>
          </ul>
        </nav>
        ...
        <div className="match-game-container">
          <img
            src={imageToBeMatched}
            className={`image-to-be-matched ${hideOrDisplay}`}
            alt="match"
          />
          <div className={`different-tabs-container ${hideOrDisplay}`}>
          ...
          <div className={`tabs-thumbnails-container ${hideOrDisplay}`}>
            <ul className="thumbnail-tabs">
              {filteredList.map(eachThunbnail => (
                <ThumbnailItem
                  details={eachThunbnail}
                  key={eachThunbnail.id}
